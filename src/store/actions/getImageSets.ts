...
export const getImageSets = createAsyncThunk(
    ...
    async() =>{
            try{
                const response :any = await api.post(`/backend/get_image_sets`, {
...
export const deleteImage = createAsyncThunk(
    ...
    async(source_filename:{source_filename:string|null}) =>{
            try{
                ...
                const response :any = await api.post(`/backend/delete_image_sets`, {
                     source_filename
          },{
                    headers: {
                        'Authorization': `Token ${localStorage.getItem("user")}` 
  ...
  export const editImage = createAsyncThunk(
    ...
    async({source_filename,save_to_filename}:{source_filename:string|null,save_to_filename:string|null}) =>{
            try{
                ...
                const response :any = await api.post(`/backend/rename_image_sets`, {
                     source_filename,
                     save_to_filename
          },{
                    headers: {
