...
const Search = (props) => {
  ...
  let { data, isLoading } = useSelector((state) => state.documentSet);
  const [DocumentList, setDocumentList] = useState([]);
  const [selectedOption, setSelectedOption] = useState("");
  const [coordinatePts, setCoordinatePts] = useState("");
  const [selectedFileName, setSelectedFileName] = useState("");
  const [FileName, setFileName] = useState("");
  const [Myoptions, setoptions] = useState(null);
  const { downloadData } = useSelector((state) => state.downloadDoc);
  const [extractDataResult, setExtractDataResult] = useState([]);
  const [resultType, setResultType] = useState("");
  const { dataDoc } = useSelector((state) => state.documents);
  const [products, setProducts] = useState([]);
  const [groupedOptions, setGroupedOptions] = useState([]);
  const [singleOpt, setsingleOpt] = useState(null);
  const [withinSearchText, withinSearchTextDropDown] = useState(null);
  //@ts-ignore
  const [matchSearchText, matchSearchTextDropDown] = useState(null);

  const [SearchText, setSearchText] = useState(null);
  const [SearchedTerm, setSearchedTerm] = useState(null);
  const [SearchTextResponse, seacrhTextData] = useState(null);
  const [showFrame, setShowFrame] = useState(false);
  const [show2, setShow2] = useState(false);
  const [showSavePageButton, setShowSavePageButton] = useState(false);
  //@ts-ignore
  const [range, rangePercentage] = useState(
    state ? state.minimum_should_match : 0
  );
  const [advanceFormState, setAdvanceFormState] = useState(false);
  const [validated, setValidated] = useState(false);
  const [selectedId, setSelectedId] = useState(false);
  const [savedSearchData, setSavedSearchData] = useState(false);
  const [showPop, setShowPop] = useState(false);
  const [showPop2, setShowPop2] = useState(false);
  const [showPopSavePage, setShowPopSavePage] = useState(false);
  const [searchFileName, setSearchFileName] = useState("");
  const [saveSearchFileName, setSaveSearchFileName] = useState("");
  const [savePageFileName, setSavePageFileName] = useState("");
  ...
  //@ts-ignore
  const [textToSearch, setTextToSearch] = useState(state ? state.text[0] : "");
  const [searchFile, setSearchFile] = useState(null);
  const [searchRange, setSearchRange] = useState(null);
  //@ts-ignore
  const [searchType, setSearchType] = useState(state ? state.type[0] : "");
  const [radioType, setRadioType] = useState("");
  const [showSaveSearchModal, setShowSaveSearchModal] = useState(false);
  const [numberOfRecords, setNumberOfRecords] = useState(8);
  const [isSaved, setIsSaved] = useState(false);
  const [selectedOptionModal, setSelectedOptionModal] = useState("");
  const [selectedPdfIndex, setSelectedPdfIndex] = useState();
  const [searchedFromSavedSearch, setsearchedFromSavedSearch] = useState(false);
  const { isPdfLoading } = useSelector((state) => state.pdfData);
  const { isCoordinateLoading } = useSelector((state) => state.coordinateData);

  //@ts-ignore

  const [arr, setArr] = useState(
    state
      ? { value: state.fileSearched, label: state.fileSearched }
  ...
  const callSearchDataGetApi = async () => {
    let savedSearchResponse = await dispatch(getSavedSearch());
  ...
  const getOptions = (arr1, arr2) => {
    ...
    let singleOptions = [];
    let sortedArray = [];
    let allDocuments = [{ value: "All Documents", label: ["All Documents"] }];
    if (arr1 && arr1.length) {
      for (let el in arr1) {
        options.push({
          value: Object.keys(arr1[el]),
          label: Object.keys(arr1[el]),
        });
      }
      setoptions(options);
    }
    if (arr2 && arr2.length) {
      ...
      sortedArray = singleOptions.sort(function (a, b) {
  ...
  useEffect(() => {
    ...
    if (data["myDocumentSets"] || "10k" in dataDoc)
      getOptions(data["myDocumentSets"], dataDoc["10k"]);
  ...
  const handleChange = (e) => {
    let documents_set = Object.values(e)[1];
    setSelectedOption(e);
    setsearchedFromSavedSearch(false);
    setSearchFile(e.label);
    if (e.label[0]) {
      if (
        e.label[0] === "All Documents" ||
        e.label[0] === "doc3" ||
        e.label[0] === "doc2" ||
        e.label[0] === "doc12"
      ) {
  ...
  const search_text = async (e) => {
    ...
    let search_text_count = await dispatch(
      searchTextCountApi({ request: request })
    );

    let totalPages = search_text_count?.payload?.data?.total_results / 10;
    setPagesCount(totalPages);
    let searchApi = await dispatch(searchTextApi({ request: request }));
    if (searchApi) {
      seacrhTextData(searchApi.payload.data);
    }
  };

  const searchTextFromSaved = async (e, item) => {
    e.preventDefault();
    setPagesCount(0);
    let myPromise = await new Promise(async (resolve, reject) => {
      try {
        ...
        await onSelectValue({
          value: item["search"].fileSearched,
          label: item["search"].fileSearched,
        });
        ...
        if (
          item["search"].fileSearched &&
          item["search"].fileSearched != "All Documents"
        ) {
          documentCondition = item["search"].fileSearched.toString().split(".");
          documentCondition.length > 1 &&
          item["search"].fileSearched != "All Documents"
    ...
    let search_text_count = await dispatch(
      searchTextCountApi({ request: myPromise })
    );

    let totalPages = Math.ceil(
      search_text_count?.payload?.data?.total_results / 10
    );

    setPagesCount(totalPages);
    let searchApi = await dispatch(searchTextApi({ request: myPromise }));
    if (searchApi) {
      seacrhTextData(searchApi.payload.data);
    }
  };

  const search_text_paginate = async (pageNumber) => {
    setShowFrame(false);
    let documentCondition = "";
  ...
  const deleteFunction = (e, searchFileName) => {
    e.stopPropagation();
  ...
  const saveNewSearchedterm = async () => {
    if (searchFileName.trim() === "") {
      ...
      let source_filename = typeName;
      let save_to_filename = searchFileName.trim();
      setNameError("");
      setShowPop2(false);
  ...
  const savePageSearched = async () => {
    let request = {
      filename: savePageFileName?.value
        ? savePageFileName.value
        : savePageFileName,
      pages: selectedDocs,
    };
    ...
    setSelectedDocs([]);
    setShowPopSavePage(false);
    let savePages = await dispatch(saveSearchPages({ request: request }));
    setselectedDoc("");
  ...
  const save_search = async (e) => {
    try {
      ...
      if (saveSearchFileName.trim() === "") {
        ...
        let saveSearch = await dispatch(saveSearchApi({ request: myPromise }));
      }
      callSearchDataGetApi();
      setShowSaveSearchModal(false);
      setCondition(false);
  ...
  const showSaveSearchPopUp = async (e) => {
    setShowSaveSearchModal(true);
    setSaveSearchFileName(selectedFileName);
  };

  const save_pages = async (e) => {
    setShowPopSavePage(true);
  };

  const savePageHandlers = (e, data, index1) => {
    e.stopPropagation();
  ...
  const displayIframe = async (item, e, index) => {
    e.stopPropagation();

    setSelectedPdfIndex(index);
    setSelectedId(item.id);
    let attributes = ["id", "PDF", "page", "page_pdf"];
    let request = {
    ...
    let request1 = {
      ...
      PDF: item["PDF"],
      page: item["page"],
      highlight_words: [[item["id"], item["highlight_words_position"]]],
    };
    let pdfLink = await new Promise(async (resolve, reject) => {
      try {
        let pdf = await dispatch(displayDocuments({ request: request }));
        resolve(pdf);
      } catch (err) {
        reject(err);
      }
    ...
    let pdfCoordinate = await new Promise(async (resolve, reject) => {
      try {
    ...
    setCoordinatePts(pdfCoordinate?.payload?.data?.data?.text);
    setHighlightWord(pdfCoordinate?.payload?.data?.data?.highlight_words);

    if (
      selectedDocs != pdfLink?.payload?.data?.data[0]?.page_pdf ||
      selectedDocs == pdfLink?.payload?.data?.data[0]?.page_pdf
    ) {
      handleClossPDF();
    }
    ...
    let pdfState = await new Promise(async (resolve, reject) => {
      try {
        let stateSeting = await setselectedDoc(
          pdfLink?.payload?.data?.data[0]?.page_pdf
        );
        resolve(stateSeting);
      } catch (err) {
        reject(err);
      ...
      <div>
        ...
        <div className="d-flex zr-search-form-leftbutton">
          ...
          <Button
            variant="primary"
            type="submit"
            onClick={() => openCloseAdvanceSearchForm}
            className="zr-srachsave clear"
          >
            Clear
          </Button>
        </div>
  ...
  const onSelectValue = (e) => {
    ...
    const stringLabel =
      typeof e?.label === "string" ? e?.label : e?.label.toString();
  ...
  return (
    <>
      {isPdfLoading || isCoordinateLoading ? <Loader /> : ""}
        ...
        <Row>
          <Col xl="8" lg="8" className="mt-4">
            ...
            <Card className="zr-searchform">
              <Form validated={validated}>
                ...
                {advanceFormState ? (
                  <div
                    className={`zr-AdSearchDiv ${
                      advanceFormState ? "zr-fadeIn" : "zr-fadeOut"
                    }`}
                  >
          ...
          <Col className="zr-savedSearchColumn mt-4" xl="4">
            ...
            <div className="zr-savedSearchs">
              {savedSearchData &&
                (savedSearchData?.data?.mySearches).map((item, index) => {
                  ...
                  return (
                    <Card
                      ...
                      onClick={(e) => {
                        searchTextFromSaved(e, item[keyName]);
                      }}
                      className="p-0 mb-3"
                    >
                      <div className="d-flex justify-content-between p-2 zr-saved-serach-head">
                        <div>
                          ...
                          <h4 className="mb-0">
                            {item[keyName]["search"].fileSearched}
                          </h4>
                        </div>
                        <div className="action-button-outer align-self-baseline mt-1">
                          <button
                            ...
                            onClick={(e) => {
                              editFunction(e, item[keyName]["description"]);
                            }}
                          >
                            <svg
                              width="22"
                          ...
                          <button
                            className="custom-action-btn"
                            onClick={(e) => {
                              deleteFunction(e, item[keyName]["description"]);
                            }}
                          >
                            <svg
                              width="16"
                      ...
                      <div className="p-2 zr-saved-search-bottom">
                        <Row>
                          <Col md="4">
                            ...
                            <p className="m-0">
                              {item[keyName]["search"].type}
                            </p>
                          </Col>
                          <Col md="4">
                            <label className="mb-2">Data Searched</label>
                            <p className="m-0">
                              {todaysDate === searchedDate
                                ? "Today"
                                : searchedDate}
                            </p>
                          </Col>
            ...
            <div className="d-flex">
              {selectedDocs.length ? (
                <div className="mb-4 mb-lg-0 me-2">
                  <Button className="zr-srachsave" onClick={save_pages}>
                    Save Page
                  </Button>
                </div>
              ) : (
            ...
            <Col xl="12">
              <Row className="">
                {SearchTextResponse && SearchTextResponse.data.length
                  ? SearchTextResponse.data.map((item, index) => {
                      ...
                      return (
                        <React.Fragment key={index}>
                          <Col lg={"6"}>
                            <div
                              style={{ height: "auto" }}
                              className={`zr-srbox bg-white w-100 p-3 mb-4 ${isActive}`}>
                              ...
                              >
                                <div className="zr-srboximage d-flex justify-content-center align-items-center">
                                  <img
                                    src={`data:image/jpeg;base64,${item.page_thumbnail_png}`}
                                    className="img-fluid"
                                    alt="Search"
                                  />
                                </div>
                                ...
                                />
                                    <span style={{cursor: 'pointer'}} className="mx-2" onClick={(e) => displayIframe(item, e, index)}>{manageDocICon}</span>
                              </div>
                              <div className="zr-srboxText">
                                <p
                                  className="zr-srboxTextPara"
      ...
      >
        ...
        <Modal.Body className="zr-custom-modal-body-dp">
          ...
          {/* <Pdftron s */}
          <Pdftron
            document={`data:application/pdf;base64,${selectedDoc}`}
            // frameIndex={index}
            textSearch={textToSearch}
            highlightText={highlightWord}
            rectCords={coordinatePts}
            closingFun={handleClossPDF}
          />
        </Modal.Body>
